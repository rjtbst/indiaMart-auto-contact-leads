...
}%cMode: ${mode}`, 'color: ' + (result.criteria.testMode !== false ? '#f39c12' : '#e74c3c') + '; font-weight: bold; font-size: 14px;');
...
function processListing(listing, criteria, listingNumber) {
  try {
    ...
    console.log(`let isScanning = false;
let scanInterval = null;
let scanCount = 0;
let sidebar = null;
...
function updateSidebarStatus() {
  chrome.storage.local.get(['isRunning', 'criteria', 'scanCount', 'logs'], (result) => {
    ...
    // Update stats
    scanCountValue.textContent = result.scanCount || 0;
    
    const logs = result.logs || [];
    logCountSidebar.textContent = logs.length;
    
    const matchCount = logs.filter(log => log.matched).length;
    matchCountValue.textContent = matchCount;
    
    const contactedCount = logs.filter(log => log.engaged === 'Just contacted').length;
    contactedCountValue.textContent = contactedCount;
...
function scanPage(criteria) {
  ...
  // Find all product listings - adjust selectors based on IndiaMArt's structure
  const listings = document.querySelectorAll('[class*="listing"], [class*="product"], [data-type="listing"], .prd, .bl_item');
...
function processListing(listing, criteria, listingNumber) {
  try {
    // Extract information from listing
    const title = extractText(listing, '[class*="title"], h2, h3, .prd-name, [class*="name"]');
    const country = extractText(listing, '[class*="country"], [class*="location"], [class*="place"]');
    const quantity = extractText(listing, '[class*="quantity"], [class*="qty"], [class*="amount"]');
    const buyer = extractText(listing, '[class*="buyer"], [class*="company"], [class*="seller"]');
    const available = extractText(listing, '[class*="available"], [class*="verification"], [class*="verified"]');
    ...
    // Check if already engaged
    const contactBtn = listing.querySelector('[class*="contact"], button[class*="btn"], [class*="enquiry"], button');
    console.log(`  Contact Button: ${contactBtn ? '✓ Found' : '✗ Not Found'}`);
    
    const engaged = contactBtn && (contactBtn.disabled || contactBtn.textContent.includes('Contacted'));
...
function startScanning() {
  ...
  chrome.storage.local.get(['criteria'], (result) => {
    if (!result.criteria) {
      alert('Please set criteria first!');
      return;
    }
    ...
    console.log('%c║  IndiaMArt Auto Contact Extension Started                ║', 'color: #2196F3; font-weight: bold;');
    console.log('%c╚═══════════════════════════════════════════════════════════╝', 'color: #2196F3; font-weight: bold;');
    console.log(`%cMode: ${mode}`, 'color: ' + (result.criteria.testMode !== false ? '#FF9800' : '#f44336') + '; font-weight: bold; font-size: 14px;');
    console.log('Criteria:', result.criteria);
    console.log('Scan interval:', formatIntervalLog(result.criteria.interval));
    console.log('');
...
function scanPage(criteria) {
  ...
  // Find all product listings - adjust selectors based on IndiaMArt's structure
  const listings = document.querySelectorAll('[class*="listing"], [class*="product"], [data-type="listing"]');
...
function processListing(listing, criteria, listingNumber) {
  try {
    ...
    console.log(`\n%c━━━ Listing #${listingNumber} ━━━`, 'color: #9C27B0; font-weight: bold;');
    console.log('%c📋 Scraped Data:', 'color: #1976d2; font-weight: bold;');
    console.log(`  Title: "${title}"`);
    console.log(`  Country: "${country}"`);
    console.log(`  Quantity: "${quantity}"`);
    console.log(`  Buyer: "${buyer}"`);
    ...
    // Skip if already contacted
    if (engaged) {
      console.log(`%c  ⏭ SKIPPED - Already contacted`, 'color: #757575;');
      return;
    }
    
    console.log('%c🔍 Matching Criteria:', 'color: #1976d2; font-weight: bold;');
    ...
    if (matched) {
      console.log(`%c  🎯 FULL MATCH!`, 'background: #4CAF50; color: white; font-weight: bold; padding: 4px 8px;');
    } else {
      console.log(`%c  ❌ NO MATCH`, 'background: #f44336; color: white; font-weight: bold; padding: 4px 8px;');
    }
    ...
    if (matched && contactBtn && !engaged) {
      if (isTestMode) {
        console.log(`%c  🧪 TEST MODE: Would click contact button (not clicking)`, 'background: #FF9800; color: white; font-weight: bold; padding: 4px 8px;');
        highlightElement(listing, true);
      } else {
        console.log(`%c  🔴 LIVE MODE: Clicking contact button...`, 'background: #f44336; color: white; font-weight: bold; padding: 4px 8px;');
        highlightElement(listing, true);
        ...
        setTimeout(() => {
          ...
          logEntry.engaged = 'Just contacted';
          updateLog(logEntry);
          console.log(`%c  ✅ Contact button clicked successfully`, 'color: #4CAF50; font-weight: bold;');
        }, 500);
      }
    } else {
      if (!matched) {
        console.log(`%c  ⛔ Not clicking - criteria not met`, 'color: #757575;');
      }
      highlightElement(listing, false);
    }
    
  } catch (error) {
    console.error(`%c❌ Error processing listing #${listingNumber}:`, 'color: #f44336; font-weight: bold;', error);
  }
}
...
// Check if we should auto-start (if running flag is set)
chrome.storage.local.get(['isRunning'], (result) => {
  if (result.isRunning) {
    setTimeout(() => {
      startScanning();
